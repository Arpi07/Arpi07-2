...
def parse_object_info():
    ...
    # integer_id 
    meta_data = {}
    with open(Data.ORIG_OBJECT_INFO_PATH, 'r') as text_file:
        for row in text_file:
            if is_header:
                ...
                meta_data[id_] = {
                    ...
                    'val': val,
                    'names': names,}
    with open(Data.OBJECT_INFO_PATH, 'wb') as pfile:
...
def edge_path_from_label_path(label_path):
    ...
    edge_name = Data.EDGE_PREFIX + label_name
...
def label_path_to_edge_saved(label_path):
    ...
    edge = Attention_cnn.train.training_utils.flat_label_to_edge_label(label, Data.N_CLASSES)
    imageio.imsave(edge_path, edge)

#labelling of boundary edge mask
def create_edge_labels():
    pool = multiprocessing.Pool(10)
    train_labels = [os.path.join(Data.TRAINING_ANNOTATION_DIR, x) for x in os.listdir(
        Data.TRAINING_ANNOTATION_DIR)]
    val_labels = [os.path.join(Data.VALIDATION_ANNOTATION_DIR, x) for x in os.listdir(
        Data.VALIDATION_ANNOTATION_DIR)]
...
def get_dataset():
    ...
    create_edge_labels()
    # Dataset dictionary
   Attention_cnn.datasets_utils.list_files(Data.DATA_DIR)
